...
void disp_adjlist(node** adj,int v,char mode)
{
	node* temp;
	int i;
	for(i=0;i<v;i++)
...
void show_distance(node** vertices, int vusr)
{
	int dist;
	dist=vertices[vusr]->d;
	printf("Distance of V%d from source is %d\n",vusr,dist);
}
int main(void)
{
	...
	printf("Enter vertex to get distance from source: ");
	scanf("%d",&vusr);
	if(vusr<0 || vusr > v)
	{
